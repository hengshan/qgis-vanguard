...
class VanguardSymbolLayer(QgsMarkerSymbolLayer):
  ...
  def clone(self):
      return VanguardSymbolLayer(self.radius)
...
class VanguardSymbolLayerWidget(QgsSymbolLayerWidget):
    ...
    def setSymbolLayer(self, layer):
        if layer.layerType() != "VanguardMarker":
            return
        self.layer = layer
...
class VanguardSymbolLayerMetadata(QgsSymbolLayerAbstractMetadata):

  def __init__(self):
    super().__init__("VanguardMarker", "My new vanguard marker", QgsSymbol.Marker)

  def createSymbolLayer(self, props):
    radius = float(props["radius"]) if "radius" in props else 10.0
    return VanguardSymbolLayer(radius)

  def createSymbolLayerWidget(self, layer):
    return VanguardSymbolLayerWidget()

fslmetadata = VanguardSymbolLayerMetadata()
